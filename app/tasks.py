...
def download_for_railway(post_id):
    set_gives_information_status('audit._sales__execution_orders', True)
    # logger.info('*'*50)
    ...
    post_dict = get_post(post_id)
    file_to_download = os.path.join(os.path.join(
        '..', 'tksva', 'app', 'static', 'files_to_download',
        str(post_dict['user_id']), post_dict['file_to_download']))
